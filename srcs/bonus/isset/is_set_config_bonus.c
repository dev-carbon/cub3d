...
int	is_set_config(t_config config, t_game *game)
{
	...
	else if (!is_set_texture(config, TSP1))
		exit_game(EXIT_FAILURE, "Texture sprite 1 is not set.", game);
	else if (!is_set_texture(config, TSP2))
		exit_game(EXIT_FAILURE, "Texture sprite 2 is not set.", game);
	else if (!is_set_texture(config, TSP3))
		exit_game(EXIT_FAILURE, "Texture sprite 3 is not set.", game);
