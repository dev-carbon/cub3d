	...
	t_setting setting)
{
	...
	while (++y < map.size.height)
	{
		...
		while (++x < map.size.width)
		{
			...
			{
				sprites[count].pos.x = setting.tile_size * (x + 0.5);
				sprites[count].pos.y = setting.tile_size * (y + 0.5);
				count++;
			}
		}
	}
...
static void	set_sprites_id(t_sprite *sprites, t_map map)
{
	int	count;
	int	x;
	int	y;
	...
	while (++y < map.size.height)
	{
		...
		{
			if (*(*(map.grid + y) + x) == MAP_SPRITE_1)
				sprites[count++].id = TSP1;
			if (*(*(map.grid + y) + x) == MAP_SPRITE_2)
				sprites[count++].id = TSP2;
			if (*(*(map.grid + y) + x) == MAP_SPRITE_3)
				sprites[count++].id = TSP3;
		}
	}
}
...
t_sprite	*set_sprites(t_game *game)
{
	...
	set_sprites_id(game->sprites, *game->map);
