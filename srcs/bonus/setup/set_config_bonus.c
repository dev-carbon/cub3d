...
static void	set(t_config *config, t_game *g)
{
	...
	else if (ft_strncmp(g->global->trim, "S1", 2) == 0 && is_valid_texture(g))
		set_texture(config, g, TSP1);
	else if (ft_strncmp(g->global->trim, "S2", 2) == 0 && is_valid_texture(g))
		set_texture(config, g, TSP2);
	else if (ft_strncmp(g->global->trim, "S3", 2) == 0 && is_valid_texture(g))
		set_texture(config, g, TSP3);
